	...
	Loop a.Length {
		...
		If (curLine) {
			...
			While (s <= len) {
				...
				lastChunk := LTrim(SubStr(curLine,s,x-y))			
...
Loop Files srcFiles "\Other\List_*.txt" ; functions and commands are combined into one list => FunctionList[]
{
	...
	While (curPos <= len And subStrEnd) {
		...
		If (curFileType = "BeginEnd") {
			...
			curObj["type"] := curType ; "function" or "command"
...
; for debug only
; ==================================================
; objType := "MapObject" ; for debug only
; testObj := MethPropList[objType]
; mList := testObj["method"]

; testList := objType "`r`n`r`n"
	...
	For level, lvlObj in ObjectCreateList {
		For label, lblObj in lvlObj {
			...
			Loop curDocArr.Length {
				...
				If (direct) {
					result := RegExMatch(curLine,regex,match)
					
					; If (InStr(curLine,"retval :=") and type = "Object") {
						; msgbox i " / " label " / " type "`r`n" curLine "`r`n`r`n" regex "`r`n`r`n" IsObject(match)
					; }
					
					If (IsObject(match) And match.Count() >= 1) {
					...
					For curObjName, curLblObj in oList {
						...
						If (curType = listType) {
							...
							result := RegExMatch(curLine,newRegex,match)
							If (IsObject(match) And match.Count() = 2) {
								obj := Map(), objName := match.Value(1)
								objNameArr := StrSplit(objName,".")
								If (objNameArr.Length > 1)
									objName := objNameArr[2]
...
ProcInput() {
	...
	For curFuncName in FunctionList {
		If (curFuncName = curPhrase) {
			curPhraseType := "function"
			Break
		}
	}
	
	If (!curPhraseType) {
		For curFuncName in CustomFunctions {
			If (curFuncName = curPhrase) {
				curPhraseType := "function"
				Break
			}
		...
		If (!doSkip) {
			...
			While (result := RegExMatch(curText,curMatch1,match,curPos)) {
				If (IsObject(match) And match.Count()) {
					...
					curSyntax := RegExReplace(curSyntax,"s)<a href[^>]*>(.*?)\Q</a>\E")
